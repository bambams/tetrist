...
void apply_input(GAME_STATE * S, INPUT_DIRECTION direction) {
...
void apply_gravity(GAME_STATE * S) {
...
void apply_movements(GAME_STATE * S) {
    GAME_PIECE * current_piece = S->current_piece;
    LINKED_LIST * list = S->pieces;
...
COLLISION * collision_spawn(GAME_PIECE * piece,
                                    POINT * origin,
                                    POINT * spot,
                                    TILE_MAP * tiles,
                                    char tile,
...
int create_block_shaded(ALLEGRO_BITMAP ** sprite,
                               ALLEGRO_COLOR fill,
...
int create_game_board(GAME_STATE * S) {
...
int create_piece_sprite(ALLEGRO_BITMAP ** sprite,
...
int detect_collisions(GAME_STATE * S, GAME_PIECE * piece,
...
void draw_pieces(LINKED_LIST ** pieces) {
...
void game_board_destroy(GAME_BOARD ** game_board) {
    assert(game_board);
...
GAME_BOARD * game_board_spawn(GAME_STATE * S) {
    int x, y;
...
int initialize_game_board(GAME_STATE * S) {
    GAME_BOARD ** game_board = &S->game_board;
...
GAME_PIECE_TYPE next_piece_type(GAME_STATE * S) {
...
void piece_destroy(GAME_PIECE ** piece) {
    assert(piece);
...
GAME_PIECE * piece_spawn(GAME_STATE * S, GAME_PIECE_TYPE type) {
    assert(type >= 0);
    assert(type < NUM_PIECES);
...
void print_collision(COLLISION * collision) {
...
void print_frame_diagnostics(GAME_STATE * S) {
    LINKED_LIST * list = S->pieces;
...
int process_logic(GAME_STATE * S) {
    S->ticks++;
...
void render_graphics(GAME_STATE * S) {
    S->redraw = 0;
...
void reset_movement(GAME_STATE * S) {
    LINKED_LIST * list = S->pieces;
...
int spawn_next_piece(GAME_STATE * S) {
